...
std::map<std::string, Tensor*> Engine::create_output_tensors_map(const std::string& graph_name, int create_mode)
{
  ...
  std::map<std::string, std::vector<int>> out_put_shapes = get_max_output_shapes(graph_name);
  std::map<std::string,Tensor*> result;
  for(int i=0;i<output_tensor_names.size();++i){
    std::vector<int> output_shape = out_put_shapes.find(output_tensor_names[i])->second;
...
void Engine::process(const std::string& graph_name, std::vector<int> core_list) {
    TRACE_POINT;
...
void Engine::process(
  ...
  _impl->output_shapes_[graph_name] = _impl->graphs_[graph_name]->inference(
      input_tensors, input_shapes, output_tensors, core_list);
}
...
void Engine::process(
  ...
  _impl->output_shapes_[graph_name] = _impl->graphs_[graph_name]->inference(
      input_tensors, output_tensors, core_list);
}
...
std::map<std::string, pybind11::array_t<float>> Engine::process(
  ...
  for (auto& item : input_tensors) {
    ...
    if (!pybind11::detail::check_flags(item.second.ptr(), pybind11::detail::npy_api::NPY_ARRAY_C_CONTIGUOUS_)) {
      int length=1;
...
void Engine::process(
  ...
  process(graph_name, inputs, outputs, core_list);
...
void Engine::process(
  ...
  process(graph_name, inputs, input_shapes, outputs, core_list);
}
#endif
