...
{
	delete this->kalman_filter;
}

vector<STrack> BYTETracker::update(const vector<vector<float>> &objects)
{
	...
	if (objects.size() > 0)
	{
		for (int i = 0; i < objects.size(); i++)
		{
			...
			tlbr_[0] = objects[i][0];
			tlbr_[1] = objects[i][1];
			tlbr_[2] = objects[i][0] + objects[i][2];
			tlbr_[3] = objects[i][1] + objects[i][3];

			float score = objects[i][4];
			int class_id = objects[i][5];
