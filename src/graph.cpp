...
public:
  ...
  void inference(std::vector<int> core_list);
...
void Graph::Graph_CC::map_output_tensors(std::map<std::string, sail::Tensor *> &output, bm_tensor_t *bm_out) {
    for (auto& item : output) {
      ...
      #ifdef IS_SOC_MODE
        bm_mem_invalidate_device_mem(handle_, &bm_out[idx].device_mem);
      #endif
...
void Graph::Graph_CC::input_s2d(
  ...
  for (auto& item : input_tensors) {
    ...
    } else if ((*input_dtypes_)[item.first] == BM_FLOAT16) {
      type_size = sizeof(float)/2;
    } else if ((*input_dtypes_)[item.first] == BM_BFLOAT16) {
      type_size = sizeof(float)/2;
...
void Graph::Graph_CC::output_d2s(
  ...
  for (auto& item : output_tensors) {
    ...
    if ((*output_dtypes_)[item.first] == BM_FLOAT32) {
      type_size = sizeof(float);
    } else if ((*output_dtypes_)[item.first] == BM_FLOAT16) {
      type_size = sizeof(float)/2;
    } else if ((*output_dtypes_)[item.first] == BM_BFLOAT16) {
      type_size = sizeof(float)/2;
...
void Graph::Graph_CC::dump_io_tensors(const std::string& name, bm_tensor_t *ins, int in_num, bm_tensor_t *outs, int out_num)
{
    ...
    for(int i = 0; i < in_num; ++i) {
        ...
        if(ins[i].shape.num_dims==3){
          tensor_w = ins[i].shape.dims[ins[i].shape.num_dims-1];
          tensor_h = ins[i].shape.dims[ins[i].shape.num_dims-2];
        }
    ...
    for(int i = 0; i < out_num; ++i) {
        ...
        if(outs[i].shape.num_dims==3){
          tensor_w = outs[i].shape.dims[outs[i].shape.num_dims-1];
          tensor_h = outs[i].shape.dims[outs[i].shape.num_dims-2];
        }
        char tmp_file_in[256];
...
void Graph::Graph_CC::inference(std::vector<int> core_list) {
    int ret = 0;
  ...
  if (core_list.empty()){
    ok = bmrt_launch_tensor_ex(p_bmrt_,
                          name_.c_str(),
                          inputs_,
                          input_tensors_.size(),
                          outputs_,
    ...
    ok = bmrt_launch_tensor_multi_cores(p_bmrt_,
                          name_.c_str(),
                          inputs_,
                          input_tensors_.size(),
                          outputs_,
    ...
    for (auto core_id : core_list) {
      bm_thread_sync_from_core(handle_, core_id);
    }
  }
#else
  ok = bmrt_launch_tensor_ex(p_bmrt_,
...
// call sync func to make sure calculation is done
...
std::map<std::string, std::vector<int>> Graph::Graph_CC::inference(
  ...
  if (io_mode_ == SYSO || io_mode_ == SYSIO) {
    ...
    for (auto& item : output) {
      int idx = output_map_[item.first];
      output_shapes[item.first].assign(bm_out[idx].shape.dims,
          bm_out[idx].shape.dims + bm_out[idx].shape.num_dims);
    }
  }
  TRACE_POINT;
...
void Graph::inference(std::vector<int> core_list) {
  return _impl->inference(core_list);
}
...
std::map<std::string, std::vector<int>> Graph::inference(
    ...
    std::map<std::string, Tensor*>&          output,
    std::vector<int>                         core_list) {
  return _impl->inference(input, input_shapes, output, core_list);
}
...
std::map<std::string, std::vector<int>> Graph::inference(
  ...
  return inference(input, input_shapes, output, core_list);
}
