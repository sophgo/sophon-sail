...
class MultiDecoderThread(object):
    def __init__(self, tpu_id, video_list, resize_type:sail.sail_resize_type, max_que_size:int, loop_count:int):
        ...
        self.yolov5_post = sail.algo_yolov5_post_1output([4, 25200, 85],640,640,10)
    ...
    def post_process(self, post_quque:queue.Queue, dete_threshold:float, nms_threshold:float):
        while (True):
            ...
            while True:
                if self.get_exit_flag():
                    break
                ret = self.yolov5_post.push_data(channels, imageidxs, tensor_with_name, dete_thresholds, nms_thresholds, ost_ws, ost_hs, padding_atrrs)
                if ret == 0:
                    break
                else:
...
if __name__ == '__main__':
    ...
    bmodel_name = "../yolov5s_int8_4b.bmodel"
    video_list = []
